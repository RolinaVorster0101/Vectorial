...
function onCanvasPointerDown(e) {
  ...
  if (state.tool === 'select') {
    if (targetId) {
      ...
      pointerState = { type: 'move', start: p, moved: false, orig: selectedShapes().map(function (s) { return JSON.parse(JSON.stringify(s)); }) };
    } else {
...
function onCanvasPointerMove(e) {
  ...
  if (pointerState.type === 'move') {
    ...
    if (!pointerState.moved) {
      const threshold = (state.viewBox.w / (El.canvasWrap.clientWidth || 900)) * 4; // ~4 screen px in SVG units
      if (Math.hypot(dx, dy) < threshold) return; // still just a click, not a drag yet — don't touch the DOM
...
function onCanvasPointerUp(e) {
  ...
  const wasNoOpClick = pointerState.type === 'move' && !pointerState.moved;
  if (['move', 'resize', 'draw', 'nodeDrag'].includes(pointerState.type) && !wasNoOpClick) pushHistory();
  pointerState = null;
  if (!wasNoOpClick) render();
