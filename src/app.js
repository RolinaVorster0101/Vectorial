...
function extractShadingColors(text) {
  const colors = [];
  ...
  for (const idx of shfillPositions) {
    ...
    const stopRe = /\/C[01]\s*\[\s*([\d.]+)\s+([\d.]+)\s+([\d.]+)\s*\]/g;
    let stopMatch;
    let r = 0, g = 0, b = 0, n = 0;
    while ((stopMatch = stopRe.exec(searchChunk))) {
      r += parseFloat(stopMatch[1]); g += parseFloat(stopMatch[2]); b += parseFloat(stopMatch[3]); n++;
    }
    if (n) {
      function hx(v) { return Math.max(0, Math.min(255, Math.round(v / n * 255))).toString(16).padStart(2, '0'); }
...
function parseEPS(text) {
  ...
  const shadingColors = extractShadingColors(bodyText);
    ...
    switch (t) {
      ...
      case 'shfill': {
        if (lastClipPath && lastClipPath.length) {
          ...
          shape.fill = shadingColors[shfillCounter] || fillColor;
          shape.stroke = 'none';
          shape.strokeWidth = 0;
          shape.fillRule = lastClipRule || 'nonzero';
          shape.name = 'EPS Gradient (approx)';
          shapes.push(shape);
        }
        shfillCounter++;
        break;
      ...
      case 'concat': { const arr = stack.pop(); if (arr && arr.__psArray && arr.vals.length === 6) { const v = arr.vals; concatMatrix(v[0], v[1], v[2], v[3], v[4], v[5]); } break; }
